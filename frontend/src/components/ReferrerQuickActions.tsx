...
function whatsappDigits(mobileNumber?: string) {
  return mobileNumber?.replace(/[^0-9]/g, '') || '';
}

interface ReferrerQuickActionsProps {
  email?: string;
  mobileNumber?: string;
  onEdit?: () => void;
  size?: 'sm' | 'md';
...
export default function ReferrerQuickActions({
  email,
  mobileNumber,
  onEdit,
  size = 'md',
  ...
  const waDigits = whatsappDigits(mobileNumber);
  const isSm = size === 'sm';

  const actionClass = isSm
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      {email && (
        ...
        >
          <svg className={isSm ? 'w-3.5 h-3.5' : 'w-4 h-4'} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {waDigits && (
        ...
        >
          <svg className={isSm ? 'w-3.5 h-3.5' : 'w-4 h-4'} viewBox="0 0 24 24" fill="currentColor">
...
export function openReferrerEmail(email: string) {
  window.location.href = `mailto:${email}`;
}

export function openReferrerWhatsApp(mobileNumber?: string) {
  const digits = whatsappDigits(mobileNumber);
  if (digits) {
