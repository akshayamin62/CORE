...
export default function StudentFormEditPage() {
  ...
  // This runs AFTER formValues are loaded to avoid overwriting them
  useEffect(() => {
    // Only set email if we have formValues (answers are loaded) and form structure
    if (studentInfo?.userId?.email && formStructure.length > 0 && Object.keys(formValues).length > 0) {
      ...
      setFormValues((prev: any) => {
        // Only update if prev has data (formValues were loaded)
        ...
        // Find and set email field in all parts (only if email is not already set or is different)
        formStructure.forEach((part) => {
          const partKey = part.part.key;
          // Skip if this part doesn't have data yet
          if (!newValues[partKey]) return;
          
          part.sections?.forEach((section) => {
            if (!newValues[partKey][section._id]) return;
            ...
            section.subSections?.forEach((subSection) => {
              ...
              if (hasEmailField) {
                instances.forEach((instance: any) => {
                  // Only update if email is not set or is different
                  if (!instance.email || instance.email !== studentEmail) {
